"""
Sim-to-real evaluation for experiment 17, restricted to the real-world
target zone (2.5-3.0V) -- same design as
../16_low_voltage_only_sim_to_real/evaluate_low_voltage_sim_to_real.py,
but sourcing synthetic data from this experiment's own simulation
(SOH 0.8-1.0, C-rate 0.1-0.2, 1.5V cutoff for both chemistries -- see
...
feature_engineering.py (exclude_final_transition=True), restrict the
feature columns used for training/testing to only those inside the
2.5-3.0V target zone.

Usage: python3 experiments/17_high_soh_low_crate_sim_to_real/evaluate_high_soh_low_crate_sim_to_real.py
"""
import matplotlib
matplotlib.use("Agg")
...
def build_combined_raw_csv():
    print(f"Loading experiment 17's high-SOH/low-C-rate synthetic data from '{SYNTHETIC_RAW_CSV}'...")
...
def main():
    ...
    print(f"\nTraining on synthetic data (SOH 0.8-1.0, C-rate 0.1-0.2, 1.5V cutoff), testing on real data -- "
    ...
    print("\nFor context: experiment 16's SAME target-zone test at SOH 0.50-0.85/C-rate 0.6 scored "
          "24.33%; experiment 07's full-curve baseline scored 24.84%.")
